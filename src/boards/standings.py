...
class Standings:
    ...
    def render(self):
        ...
        if self.data.config.preferred_standings_only:
            if type == 'conference':
                print("conf1")
                conf = getattr(self.data.standings.by_conference, self.data.config.preferred_conference)
                
            elif type == 'division':
                print("div1")
                div = getattr(self.data.standings.by_division, self.data.config.preferred_divisions)
                print(div)
        else:
            if type == 'conference':
...
def draw_records(data, name, records, img_height, width, i):
    """
    ...
    draw.text((1, 0 + i), name, font=layout.font)
    row_pos += row_height
    ...
    for team in records:
        ...
        draw.rectangle([0, top + row_pos + i, 12, row_pos + i], fill=(bg_color['r'], bg_color['g'], bg_color['b']))
        draw.text((1, row_pos + i), abbev, fill=(txt_color['r'], txt_color['g'], txt_color['b']), font=layout.font)
        draw.text((57, row_pos + i), points, font=layout.font)
        draw.text((19, row_pos + i), "{}-{}-{}".format(wins, losses, ot), font=layout.font)
        row_pos += row_height
